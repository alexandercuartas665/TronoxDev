...
using Tronox.Application;
using Tronox.Application.Common;
using Tronox.Application.Common.Auth;
using Tronox.Domain.Enums;
using Tronox.Infrastructure;
...
builder.Services.AddScoped<ITenantContext, Tronox.Web.Auth.AmbientTenantContext>();
...
if (!app.Environment.IsDevelopment())
{
    ...
    // En produccion las migraciones NO se aplican solas. Si TRONOX_RUN_MIGRATIONS=true
    ...
    if (string.Equals(Environment.GetEnvironmentVariable("TRONOX_RUN_MIGRATIONS"), "true", StringComparison.OrdinalIgnoreCase))
    {
        using var scope = app.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<TronoxDbContext>();
    ...
    IPasswordHasher hasher) =>
{
    ...
    // interno "Plataforma TRONOX") recibe los dos claims y puede usar tanto la consola de gobierno
...
app.MapPost("/auth/register", async (
    ...
    [FromForm] string email,
    [FromForm] string password,
    Tronox.Application.Auth.ISelfSignupService signup) =>
{
    var result = await signup.SignUpAsync(
        new Tronox.Application.Auth.SelfSignupRequest(agencyName, displayName, email, password));
...
app.MapPost("/auth/activate", async (
    ...
    Tronox.Application.Auth.IAccountActivationService activation) =>
{
...
app.MapPost("/auth/resend-activation", async (
    [FromForm] string email,
    Tronox.Application.Auth.IAccountActivationService activation,
    Tronox.Application.Common.IEmailSender emailSender,
    Tronox.Application.Admin.IPlatformBrandingService branding) =>
{
...
app.MapPost("/auth/forgot", async (
    HttpContext http,
    [FromForm] string email,
    Tronox.Application.Auth.IPasswordResetService reset) =>
{
...
app.MapGet("/connect/google", async (
    ...
    Tronox.Application.Auth.IGoogleSignInService google) =>
{
    ...
    TronoxDbContext db) =>
{
    ...
    // el usuario es miembro de algun tenant (caso Super Admin con tenant interno "Plataforma TRONOX").
