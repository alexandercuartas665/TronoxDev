// ============================================================================
// tronox-bpmn.js - Interop Blazor <-> bpmn-js (v8.8.2, self-hosted) para el
// EDITOR de flujos. Reemplaza el canvas propio por bpmn-js embebido. SOLO
// editor (modeler); el viewer de ejecucion es otra ola.
//
...
function injectStyle() {
    if (document.getElementById('tronox-bpmn-style')) { return; }
    const css =
        ...
        '.tronox-node-note { width: 168px; box-sizing: border-box; font: 500 11px system-ui, sans-serif;' +
        ...
        '.tronox-node-note-pin { display: inline-block; width: 6px; height: 6px; border-radius: 50%;' +
        ' background: #d98a00; margin-right: 5px; vertical-align: middle; }';
    const style = document.createElement('style');
    style.id = 'tronox-bpmn-style';
...
export async function init(containerId, dotnetRef, xml) {
    if (typeof window.BpmnJS !== 'function') {
        console.error('[tronox-bpmn] window.BpmnJS no esta cargado (revisa lib/bpmnio/bpmn-modeler.js).');
        return false;
    }
    destroy(containerId);
    ...
    if (!container) {
        console.error('[tronox-bpmn] contenedor no encontrado:', containerId);
        return false;
    }
    ...
    try {
        ...
        console.error('[tronox-bpmn] no se pudo importar el XML BPMN:', err);
    }
...
export async function exportXml(containerId) {
    ...
    try {
        ...
        console.error('[tronox-bpmn] no se pudo exportar el XML:', err);
        return null;
    }
}
...
export async function importXml(containerId, xml) {
    ...
    try {
        ...
        console.error('[tronox-bpmn] no se pudo importar el XML:', err);
        return false;
    }
}
...
export function applyNodeNotes(containerId, items) {
    ...
    items.forEach(function (it) {
        ...
        try { overlays.remove({ element: it.id, type: 'tronox-note' }); } catch (err) { /* sin overlay previo */ }
        const text = (it.note || '').trim();
        if (!text) { return; }
        const safe = text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        overlays.add(it.id, 'tronox-note', {
            position: { bottom: -8, left: 0 },
            html: '<div class="tronox-node-note" title="' + safe + '">'
                + '<span class="tronox-node-note-pin"></span>' + safe + '</div>'
        });
    });
}
