...
/// <summary>
/// Implementacion de IFormTokenService (RQ08, port ECOREX / ADR-0015). El token opaco son 32 bytes
/// aleatorios (CSPRNG) en base64url; se persiste SOLO su SHA-256 (hex, 64). IMPORTANTE tenant
/// scoping: ValidateAsync usa IgnoreQueryFilters porque el visor anonimo NO tiene tenant en
/// contexto; es el unico cross-tenant permitido del modulo, acotado a la igualdad exacta del hash
/// (imposible de enumerar), y devuelve el TenantId del token para que el visor fije el ambiente
/// antes de tocar cualquier otro servicio. El resto de operaciones (emitir, revocar, marcar usado,
/// listar) son tenant-scoped.
/// </summary>
public sealed class FormTokenService : IFormTokenService
{
    ...
    public async Task<FormResult<EmitFormTokenResult>> EmitAsync(long definitionId, EmitFormTokenRequest options, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<FormTokenValidation> ValidateAsync(string token, CancellationToken cancellationToken = default)
    {
        ...
        // UNICO punto cross-tenant permitido (ADR-0015): el visor anonimo no tiene tenant en
        // contexto, asi que la busqueda ignora el filtro global PERO solo por el hash exacto del
        // token presentado. El TenantId devuelto es el que el visor debe fijar como ambiente para
        ...
        // La definicion debe seguir Active (no retirada / no archivada); si no, token neutro-invalido.
        var isActive = await _db.FormDefinitions.IgnoreQueryFilters().AsNoTracking()
            .AnyAsync(d => d.Id == entity.DefinitionId
                && d.Status == FormStatus.Active && !d.IsArchived, cancellationToken);
        if (!isActive)
        {
            return FormTokenValidation.Invalid;
        }
    ...
    public async Task<FormResult<bool>> RevokeAsync(long tokenId, CancellationToken cancellationToken = default)
    {
    ...
    {
        // Tenant-scoped: el visor publico ya fijo el ambiente del tenant del token.
        var entity = await _db.FormTokens.FirstOrDefaultAsync(t => t.Id == tokenId, cancellationToken);
    ...
    public async Task<IReadOnlyList<FormTokenDto>> ListAsync(long definitionId, CancellationToken cancellationToken = default)
