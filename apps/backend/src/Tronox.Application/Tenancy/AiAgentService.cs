...
/// <summary>CRUD de agentes de IA del tenant (RQ16, port de ECOREX). Ver IAiAgentService.</summary>
public sealed class AiAgentService : IAiAgentService
{
    ...
    public async Task<AiAgentDetailDto?> GetAsync(long id, CancellationToken cancellationToken = default)
    {
        ...
        var resources = await _db.AiAgentResources.AsNoTracking()
            ...
            .Select(r => MapResource(r)).ToListAsync(cancellationToken);
        var prompts = await _db.AiAgentPrompts.AsNoTracking()
            .Where(p => p.AgentId == id).OrderBy(p => p.SortOrder)
            .Select(p => MapPrompt(p)).ToListAsync(cancellationToken);
        return new AiAgentDetailDto(Map(agent, resources.Count), resources, prompts);
    }

    public async Task<AiAgentDto?> CreateAsync(CreateAiAgentRequest request, long actorUserId, CancellationToken cancellationToken = default)
    {
        ...
        _audit.Write(actorUserId, "ai-agent.create", nameof(AiAgent), agent,
            previousValue: null, newValue: new { agent.Name, agent.Provider }, tenantId: tenantId);
        await _db.SaveChangesAsync(cancellationToken);
        return Map(agent, 0);
    }

    public async Task<AiAgentDto?> UpdateAsync(long id, UpdateAiAgentRequest request, long actorUserId, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<AiAgentDto?> SetActiveAsync(long id, bool active, long actorUserId, CancellationToken cancellationToken = default)
    {
        var agent = await _db.AiAgents.FirstOrDefaultAsync(a => a.Id == id, cancellationToken);
        if (agent is null) { return null; }
        agent.IsActive = active;
        _audit.Write(actorUserId, active ? "ai-agent.activate" : "ai-agent.deactivate", nameof(AiAgent), agent,
    ...
    public async Task<bool> DeleteAsync(long id, long actorUserId, CancellationToken cancellationToken = default)
    {
        ...
        _audit.Write(actorUserId, "ai-agent.delete", nameof(AiAgent), agent,
            previousValue: new { agent.Name }, newValue: null, tenantId: agent.TenantId);
        await _db.SaveChangesAsync(cancellationToken);
        return true;
    }

    public async Task<AiAgentResourceDto?> AddResourceAsync(CreateAgentResourceRequest request, long actorUserId, CancellationToken cancellationToken = default)
    {
        if (_tenantContext.TenantId is not long tenantId) { return null; }
    ...
    public async Task<AiAgentResourceDto?> UpdateResourceAsync(long id, UpdateAgentResourceRequest request, long actorUserId, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<bool> DeleteResourceAsync(long id, long actorUserId, CancellationToken cancellationToken = default)
    {
        var res = await _db.AiAgentResources.FirstOrDefaultAsync(r => r.Id == id, cancellationToken);
    ...
    public async Task<AiAgentPromptDto?> AddPromptAsync(CreateAgentPromptRequest request, long actorUserId, CancellationToken cancellationToken = default)
    {
        if (_tenantContext.TenantId is not long tenantId) { return null; }
        var agent = await _db.AiAgents.FirstOrDefaultAsync(a => a.Id == request.AgentId, cancellationToken);
        if (agent is null) { return null; }
    ...
    public async Task<AiAgentPromptDto?> UpdatePromptAsync(long id, UpdateAgentPromptRequest request, long actorUserId, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<bool> DeletePromptAsync(long id, long actorUserId, CancellationToken cancellationToken = default)
    {
        var prompt = await _db.AiAgentPrompts.FirstOrDefaultAsync(p => p.Id == id, cancellationToken);
    ...
    public async Task<AiAgentDto?> DuplicateAsync(long sourceId, long actorUserId, CancellationToken cancellationToken = default)
    {
        if (_tenantContext.TenantId is not long tenantId) { return null; }
        ...
        // NO se copian los valores de cache (datos de conversacion).
        _audit.Write(actorUserId, "ai-agent.duplicate", nameof(AiAgent), copy,
    ...
    public async Task<IReadOnlyList<AiAgentPromptVersionDto>> GetPromptHistoryAsync(long agentId, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<AiAgentDetailDto?> RestorePromptVersionAsync(long agentId, int versionIndex, long actorUserId, CancellationToken cancellationToken = default)
    {
        ...
        _audit.Write(actorUserId, "ai-agent.restore-prompt-version", nameof(AiAgent), agent,
