...
public sealed class AuthEndpointsTests : IClassFixture<TronoxApiFactory>
{
    private readonly TronoxApiFactory _factory;

    public AuthEndpointsTests(TronoxApiFactory factory) => _factory = factory;

    [Fact]
    public async Task Login_SingleTenantUser_ReturnsTokenWithTenant()
    {
        var client = _factory.CreateClient();
        var token = await LoginAsync(client, TronoxApiFactory.SingleEmail);
    ...
    public async Task Login_WrongPassword_Returns401()
    {
        ...
        var response = await client.PostAsJsonAsync(
            "/connect/token",
            new LoginRequest(TronoxApiFactory.SingleEmail, "clave-incorrecta"));
    ...
    {
        var client = _factory.CreateClient();
        var token = await LoginAsync(client, TronoxApiFactory.MultiEmail);
    ...
    public async Task TenantConfigurations_AreIsolatedByTokenTenant()
    {
        ...
        var tokenA = await LoginAsync(client, TronoxApiFactory.SingleEmail);
        ...
        var tokenMulti = await LoginAsync(multi, TronoxApiFactory.MultiEmail);
    ...
    {
        var client = _factory.CreateClient();
        var token = await LoginAsync(client, TronoxApiFactory.SuperEmail);
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token.AccessToken);
