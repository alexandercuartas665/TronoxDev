...
public sealed class TenantUsersTests : IClassFixture<TronoxApiFactory>
{
    private readonly TronoxApiFactory _factory;

    public TenantUsersTests(TronoxApiFactory factory) => _factory = factory;
    ...
    private async Task<HttpClient> TenantAdminClientForBAsync()
    {
        var client = _factory.CreateClient();
        var login = await client.PostAsJsonAsync("/connect/token",
            new LoginRequest(TronoxApiFactory.MultiEmail, TronoxApiFactory.Password));
        var token = (await login.Content.ReadFromJsonAsync<TokenResponse>())!;
