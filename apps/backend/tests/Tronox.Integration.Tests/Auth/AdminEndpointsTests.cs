...
public sealed class AdminEndpointsTests : IClassFixture<TronoxApiFactory>
{
    private readonly TronoxApiFactory _factory;

    public AdminEndpointsTests(TronoxApiFactory factory) => _factory = factory;
    ...
    public async Task AdminEndpoint_ForTenantUser_Returns403()
    {
        ...
        var response = await client.PostAsJsonAsync("/connect/token",
            new LoginRequest(TronoxApiFactory.SingleEmail, TronoxApiFactory.Password));
    ...
    private async Task<HttpClient> SuperAdminClientAsync()
    {
        var client = _factory.CreateClient();
        var response = await client.PostAsJsonAsync("/connect/token",
            new LoginRequest(TronoxApiFactory.SuperEmail, TronoxApiFactory.Password));
