...
Dos clases base en `Ecorex.Domain`:
...
En `Ecorex.Domain` (o `Ecorex.Shared` si el frontend los necesita). Se persisten como **texto** en PostgreSQL (no int) para legibilidad y estabilidad ante reordenamientos; columnas `varchar`.
...
## Punto 6 - EcorexDbContext

Vive en `Ecorex.Infrastructure`. Provider Npgsql.

Componentes:

1. **DbSets** para todas las entidades de los puntos 3 y 4.
2. **`ITenantContext`** (interfaz en `Ecorex.Application`, implementacion en Infrastructure/Api): expone `Guid? TenantId` y `Guid? UserId` del request actual. Resuelto desde el claim `tenant_id` del JWT (ver Notas dev). En workers/seed puede no haber tenant.
...
8. **Migraciones**: EF Core Migrations contra la BD Docker local (`Host=localhost;Port=5442;Database=ecorex_dev`). Carpeta `apps/backend/src/Ecorex.Infrastructure/Migrations`.
...
Proyecto `Ecorex.Integration.Tests`. **Bloqueante**: ningun modulo tenant-scoped avanza sin esto verde.
...
- Alternativa local rapida: apuntar a la BD `ecorex_dev` del compose, pero se prefiere Testcontainers para no ensuciar datos.
...
4. `EcorexDbContext` + naming + filtros + interceptor + configuraciones en Infrastructure.
